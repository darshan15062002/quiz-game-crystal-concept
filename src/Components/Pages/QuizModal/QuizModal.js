...
const QuizModal = ({ title, questions, setShowAns, setResultShow, handleNext, handlePrev, count, selectedOptions }) => {
    ...
    return (
        <div className=" overflow-y-auto overflow-x-hidden bg-slate-50 bg-transparent/10 fixed flex top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0  h-full">
                    ...
                    <div className="answer flex flex-col justify-center gap-y-1">

                        {
                            questions[count]?.answers?.map((item, index) => (
                                ...
                                <div className={`text-black border px-2 rounded-md border-black flex gap-4 ${index + 1 === Number(selectedOptions[count]) && String(selectedOptions[count]) !== questions[count]?.correctAnswer && 'bg-red-400'} ${String(index + 1) === questions[count]?.correctAnswer && 'bg-green-400'}
                                    `}>


                                    <input type="radio" name="answer" id={"answer"} value={item} checked={false} readOnly />
